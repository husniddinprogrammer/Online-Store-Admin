...
export function ProductImageUpload({
  ...
  const retryFile = useCallback((id: string) => {
    setFiles((prev) =>
      prev.map((f) =>
        f.id === id ? { ...f, status: "idle", error: undefined, progress: 0 } : f
      )
    );
  }, []);
  ...
  const uploadAll = async () => {
    ...
    results.forEach((result, i) => {
      if (result.status === "fulfilled") {
        ...
        const message =
          axiosError.response?.data?.detail ?? axiosError.message ?? "Upload failed";
        setFiles((prev) =>
          prev.map((f) =>
            f.id === pending[i].id
      ...
      {files.length > 0 && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <p className="text-xs text-muted-foreground tabular-nums">
            ...
            {successCount > 0 && (
              <span className="text-success ml-2">{successCount} done</span>
            )}
            {errorCount > 0 && (
...
function FileRow({ entry, showMainBadge, onRemove, onRetry, disableRemove }: FileRowProps) {
  const isValidationError =
    entry.error?.includes("Only") || entry.error?.includes("too large");
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        {entry.status === "error" && entry.error && (
          <div className="flex items-center gap-2">
            <p className="text-xs text-destructive leading-none flex-1 truncate">
              {entry.error}
            </p>
            {!isValidationError && (
              <button
